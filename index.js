...
(function() {
  var AWS, app, async, colors, currentProject, endpoints, express, fetchDeployedTranslations, fetchStorms, i, numberOfFields, parseProject, phantom, port, projectList, projects, request, s3, walkTranslations, workspace;
  ...
  endpoints = (function() {
    ...
    for (i = _i = 1; _i <= 4; i = ++_i) {
      _results.push("https://api.zooniverse.org/projects/cyclone_center/groups/categories/2005?page=" + i);
    }
    return _results;
  })();

  fetchStorms = function(item, callback) {
    return request.get(item, function(err, res, body) {
      var reducedData;
      body = JSON.parse(body);
  ...
  app.get('/storms_status', function(req, res) {
    return async.map(endpoints, fetchStorms, function(err, groups) {
      var reducedData;
